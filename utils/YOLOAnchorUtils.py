...
def saveVisHeatMap(predict, image, W, H, layer, input_shape, cut_region, save_vis_path):
    ...
    heat_map = F.sigmoid(predict[..., 5]).numpy()
...
def saveClsHeatMap(predict, box_classes, image, W, H, layer, input_shape, cut_region, save_vis_path):
    '''提取clsmap(obj置信度 * 类别相关的置信度)
    '''
    heat_map = torch.max(F.sigmoid(predict), -2)[0][..., 6:]
...
def YOLOv5BestRatioAssigner(targets, input_shape, anchors, anchors_mask, bbox_attrs, threshold=4):
    ''' - YOLOv5正负样本分配策略(根据gt与anchors的长宽比例, 是基于anchor_base的静态分配方法).
        ...
        # Args
            ...
            - bbox_attrs:      6 + num_cls
            - threshold:       anchor作为正样本时和GT的长宽比在 (1/threshold~threshold) 之间

        # Returns
            - output_targets:  分配结果 [[...],[...],[...]] output_targets[i].shape = (3, w, h, 6+cls_num)
    ...
    # 初始化 output_targets = [[...],[...],[...]] 
    # 为每个特征层的每个锚框预留空间存储预测目标 output_targets[i].shape = (3, 20, 20, 6+cls_num), (3, 40, 40, 6+cls_num), (3, 80, 80, 6+cls_num) 6:(x0, y0, x1, y1, theta)
    ...
    for l in range(num_layers):
        ...
        scaled_targets = np.zeros_like(targets) # [num_gts, 6]
        # 对每个真实框计算其在当前特征层的位置
        scaled_targets[:, [0,2]] = targets[:, [0,2]] * feat_w
        scaled_targets[:, [1,3]] = targets[:, [1,3]] * feat_h
        # 将角度归一化到(0,1)之间 (角度范围在[-180, 0))
        scaled_targets[:, 4] = (targets[:, 4] + 180) / 180
        # 类别标签则不做处理
        scaled_targets[:, 5] = targets[:, 5]
        ...
        for gt_id, ratio in enumerate(max_ratios):
            ...
            for k, mask in enumerate(anchors_mask[l]):
                ...
                # offset添加了当前GT点的两个最近邻点,这样一个GT就对应3个正样本(这一步为了增加正样本数量)
                ...
                for offset in offsets:
                    ...
                    # 真实框的种类
                    c = int(scaled_targets[gt_id, 5])
                    # cx, cy, w, h, theta
                    output_targets[l][k, local_j, local_i, :5] = scaled_targets[gt_id, :5]  
                    # obj_map=1, 表示对应位置存在目标
                    output_targets[l][k, local_j, local_i, 5] = 1     
                    # 对应类别置为1, 其余为0(one-hot label)
                    output_targets[l][k, local_j, local_i, c + 6] = 1  
                    # 记录当前网格点的最佳匹配IOU
...
def inferDecodeBox(inputs, input_shape, num_classes, anchors, anchors_mask):
    ...
    for i, input in enumerate(inputs):
        ...
        prediction = input.view(batch_size, len(anchors_mask[i]), 6 + num_classes, input_height, input_width).permute(0, 1, 3, 4, 2).contiguous()
        ...
        pred_boxes = YOLOv5Reg2Box(batch_size, i, dx, dy, dh, dw, theta, anchors_mask, scaled_anchors, input_height, input_width)
        # 将输出结果转换成归一化坐标(0, 1)之间, 角度theta不用转换
        _scale = torch.Tensor([input_width, input_height, input_width, input_height, 1]).type_as(dx)
        # output.shape = [bs, num_anchors, 5+num_cls]
        output = torch.cat((pred_boxes.view(batch_size, -1, 5) / _scale, conf.view(batch_size, -1, 1), pred_cls.view(batch_size, -1, num_classes)), -1)
        outputs.append(output.data)
    return outputs



def YOLOv5Reg2Box(bs, l, dx, dy, dh, dw, theta, anchors_mask, scaled_anchors, in_h, in_w):
    '''YOLOv5将offset作用到anchor上进行解码得到最终预测结果
    '''
    # 当训练时调用该方法theta=False; 当推理时theta为归一化角度,需转化为实际的角度[-180, 0)
    ...
    pred_boxes = torch.zeros((bs, 3, in_w, in_h, 5), device=dx.device)
    pred_boxes[..., 0] = grid_x + dx
    pred_boxes[..., 1] = grid_y + dy
    pred_boxes[..., 2] = anchor_w * dw
    pred_boxes[..., 3] = anchor_h * dh
    # 归一化角度还原回原始的角度, 范围在[-180, 0)
    pred_boxes[..., 4] = theta * 180 - 180
    # pred_boxes.shape = [bs, 3, w, h, 5]
    return pred_boxes
...
def rotatedNMS(prediction, input_shape, conf_thres=0.5, nms_thres=0.4, agnostic=False):
    '''推理一张图/一帧
        # Args:
    ...
    for i, image_pred in enumerate(prediction):
        ...
        cls_score, cls_id = torch.max(image_pred[:, 6:], 1, keepdim=True)
        '''首先筛选掉置信度小于阈值的预测 '''
        # class_conf  [num_anchors]
        conf_keep = (image_pred[:, 5] * cls_score[:, 0] >= conf_thres).squeeze()
...
def BBoxesKmeans2AnchorsDOTA(ann_dir, seed=0):
    '''对数据集进行聚类得到anchor的先验尺寸
    '''
    ...
    for ann_name in tqdm(os.listdir(ann_dir)):
        ...
        with open(ann_path, 'r') as txt:
            for line in txt.readlines():
                line = line.split(' ')
                box_w_h = np.array([float(box) for box in line[3:5]])*1024
                datas.append(box_w_h)
...
if __name__ == '__main__':
    ...
    ann_path = 'E:/datasets/RemoteSensing/DOTA-1.0_ss_1024/train/yolo_longside_format_annfiles'
    datas, centers, labels = BBoxesKmeans2AnchorsDOTA(ann_path, seed=23)
