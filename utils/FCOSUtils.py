...
# def FCOSAssigner(gt_boxes, gt_angles, gt_labels, input_shape, strides=[8, 16, 32, 64, 128], \
#                  limit_ranges=[[-1,64],[64,128],[128,256],[256,512],[512,999999]], sample_radiu_ratio=1.5):
#     '''FCOS正负样本分配
#         Args:
#             - gt_boxes:           GT box   [bs, max_box_nums, 4]
#             - gt_angles:          GT angle [bs, max_box_nums,  ]
#             - gt_labels:          GT label [bs, max_box_nums,  ]
...
def FCOSAssigner(gt_boxes, gt_angles, gt_labels, input_shape, strides=[8, 16, 32, 64, 128], \
    ...
    for level in range(len(strides)):
        ...
        '''高斯分配'''
        # 扩展 gt_boxes 的宽和高，使其与 c_left_off 等张量兼容
        gt_w = (gt_boxes[..., 2] - gt_boxes[..., 0]).unsqueeze(1).expand(bs, h_mul_w, gt_boxes.size(-2))
        gt_h = (gt_boxes[..., 3] - gt_boxes[..., 1]).unsqueeze(1).expand(bs, h_mul_w, gt_boxes.size(-2))
        # 旋转角度
        cos_angle = torch.cos(torch.deg2rad(gt_angles)).unsqueeze(1).expand(bs, h_mul_w, gt_boxes.size(-2))
        sin_angle = torch.sin(torch.deg2rad(gt_angles)).unsqueeze(1).expand(bs, h_mul_w, gt_boxes.size(-2))
        # 基于旋转框计算旋转矩阵
        rotation_matrix = torch.stack([cos_angle, -sin_angle, sin_angle, cos_angle], dim=-1).reshape(bs, h_mul_w, gt_boxes.size(-2), 2, 2)
        # 计算相对于目标中心的偏移量
        offsets = torch.stack([c_left_off, c_top_off], dim=-1).unsqueeze(-1)  # 添加维度用于矩阵乘法
        rotated_offsets = torch.matmul(rotation_matrix, offsets).squeeze(-1)  # 旋转后的偏移量
        ...
        # for bs, cnt_map in enumerate(cnt_targets.reshape(bs, feat_h, feat_w).cpu().numpy()):
        #     plt.imshow(cnt_map)
        #     plt.savefig(f'bs-{bs}_layer-{level}.png')
