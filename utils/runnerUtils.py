...
def loadDatasets(mode:str, seed:int, bs:int, num_workers:int, my_dataset:dict):
    ...
    val_img_dir = my_dataset['val_dataset']['img_dir']
    imgset_file_path = my_dataset['imgset_file_path']
    eval_ann_dir = my_dataset['eval_ann_dir']
    # 固定每个方法里都有一个DOTA2LongSideFormatYOLODataset
...
def recoardArgs(
    ...
    if mode == 'train':
        ...
        for loss_name, loss_value in loss.items():
            argsHistory.record(loss_name, float(loss_value))
